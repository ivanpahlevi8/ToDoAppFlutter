...
abstract interface class AuthRemoteDatasource {
  ...
  TaskEither<BaseException, ResponseModel<UserModel>> register(
    RegisterFieldModel registerField,
  );
}
...
class AuthRemoteDatasourceImpl extends BaseNetwork
  ...
  TaskEither<BaseException, ResponseModel<UserModel>> register(
    ...
    final getApiUrl = apis.getRegisterUri();

    print("DEBUG URL: ${apis.getRegisterUri().toString()}");

    return service
        .post(getApiUrl, null, body: registerField.toJson())
        // Ensure the status code is 200-299
        .flatMap(
          (response) => TaskEither.fromEither(validator.validateBody(response)),
        )
